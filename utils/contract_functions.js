...
async function contract_redeem(web3, contract_address,  gas_price) {
    try {
        const contract = new web3.eth.Contract(contract_abi, contract_address);

        const sentTx = await contract.methods.redeem().send({ gas: 100000, gasPrice:  web3.utils.toWei(gas_price.toString(), 'gwei') });

    } catch (e) {
        console.log("------------------------------------");
        console.log("ERROR in: contract_redeem()")
        console.log(e);
    }
}
